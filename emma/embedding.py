...
class AliEmbedding:
    ...
    def call(self, url: str):
        print(url)
        result = dashscope.BatchTextEmbedding.call(
            dashscope.BatchTextEmbedding.Models.text_embedding_async_v2,
            url=url,
            text_type="document",
        )
        return result
    ...
    def embedding(self, inputs: List):
        ...
        if len(inputs) == 1:
            resp = dashscope.TextEmbedding.call(
                model=dashscope.TextEmbedding.Models.text_embedding_v2, input=inputs[0]
            )
...
class LocalEmbedding(embedding_query_pb2_grpc.EmbeddingServiceServicer):
    def __init__(self, model_path: str = None) -> None:
        self.model_path = (
            model_path or "/home/chenxueliang/embedding/xiaobu-embedding-v2"
        )
    ...
    def GetEmbeddings(self, request, context):
        ...
        return embedding_query_pb2.EmbeddingResponse(
            serialized_embeddings=buffer.getvalue().to_pybytes()
        )
    ...
    def embeddings(cls, model="xiaobu-embedding-v2", inputs: List = None) -> np.array:
        ...
        channel_credentials = composite_channel_credentials(
            ssl_credentials, auth_credentials
        )

        with grpc.secure_channel(
            os.getenv("EMBEDDING_SERVER"), channel_credentials, options=channel_options
        ) as channel:
            stub = embedding_query_pb2_grpc.EmbeddingServiceStub(channel)
            response = stub.GetEmbeddings(
                embedding_query_pb2.EmbeddingQuery(queries=inputs)
            )
...
if __name__ == "__main__":
    ...
    embedding_query_pb2_grpc.add_EmbeddingServiceServicer_to_server(
        LocalEmbedding(), server
    )
